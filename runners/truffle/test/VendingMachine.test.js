...
contract("VendingMachine", (accounts) => {
  ...
  it(`Ensures that the starting balance of the vending machine is ${INITIAL_BALANCE}, the initial balance`, async () => {
    ...
    assert.equal(currentBalance, actualBalance, `The initial balance should be ${currentBalance} donuts`)
  })
  ...
  it(`Ensures the balance of the vending machine can be updated`, async () => {
    ...
    const actualBalance = await instance.getVendingMachineBalance()
    assert.equal(currentBalance, actualBalance, `The balance should be ${currentBalance} donuts after restocking`)
  })
  ...
  it(`Allows donuts to be purchased`, async () => {
    ...
    const actualBalance = await instance.getVendingMachineBalance()
    assert.equal(currentBalance, actualBalance, `The balance should be ${currentBalance} donuts after sale`)
  })
  ...
  it(`Allows multiple accounts to purchase donuts`, async () => {
    ...
    const actualBalance = await instance.getVendingMachineBalance()
    assert.equal(currentBalance, actualBalance, "The balance should be updated after multiple purchases")
